---
tags:
  - text
  - editor
  - pluma
  - graphical
  - GUI
  - pelle
---

# Pluma

Pluma is a simple graphical [text editor](text_editors.md)
and is the default on the Pelle HPC cluster.

Here is what Pluma looks like:

![Pluma](img/pluma.png)

In Pelle's remote desktop environment, you can start Pluma by clicking here:

![How to start Pluma](img/pluma_click_from_main_menu.png)

Here is what Pluma looks like when actually showing some text:

![Pluma with content](img/pluma_with_content.png)

Finding Pluma's 'About' dialog:

![Pluma, click on 'About'](img/pluma_click_about.png)

Pluma's 'About' dialog:

![Pluma, showing the 'About' dialog](img/pluma_showing_about.png)
